...
В расчет добавлено явное разделение режимов работы энергообъекта:

```text
auto   — режим определяется автоматически по температуре;
winter — принудительный зимний режим;
summer — принудительный летний режим.
```

В GUI режим выбирается в поле `Режим работы`. В API параметр называется `operation_mode`.

В расчете режим влияет на:

- интегральную теплоотводящую способность;
- коэффициент собственных нужд;
- итоговые КПД и удельный расход топлива.

В результате расчета возвращается фактически примененный режим `operation_mode`.
...
## Исправление цветовой схемы GUI

Добавлено явное задание цветов текста и палитры Qt, чтобы на системах с тёмной темой текст не отображался белым на светлом фоне.
